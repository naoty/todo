require "spec_helper"
require "stringio"

RSpec.describe Todo::Commands::Delete do
  describe "#run" do
    let(:output) { StringIO.new }
    let(:error_output) { StringIO.new }
    let(:repository) { instance_double(Todo::FileRepository) }

    before do
      allow(repository).to receive(:list).and_return([])
    end

    context "when arguments are empty" do
      it "puts help message to error output" do
        delete = described_class.new(arguments: [], output: output, error_output: error_output)
        delete.run(repository: repository)
      rescue SystemExit
        # ignore exit
      ensure
        expect(error_output.string).to eq(described_class::HELP_MESSAGE)
      end

      it "exits with status code 1" do
        delete = described_class.new(arguments: [], output: output, error_output: error_output)
        expect {
          delete.run(repository: repository)
        }.to raise_error(an_instance_of(SystemExit).and(having_attributes({status: 1})))
      end
    end

    ["-h", "--help"].each do |flag|
      context "when arguments include '#{flag}' flag" do
        it "puts help message to output" do
          delete = described_class.new(arguments: [flag], output: output, error_output: error_output)
          delete.run(repository: repository)
          expect(output.string).to eq(described_class::HELP_MESSAGE)
        end
      end
    end

    context "when arguments include IDs" do
      it "calls Todo::FileRepository#delete" do
        delete = described_class.new(arguments: ["1"], output: output, error_output: error_output)
        expect(repository).to receive(:delete).with(ids: [1])
        delete.run(repository: repository)
      end
    end
  end
end
