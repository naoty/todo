  ...
  describe "#run" do
    ...
    context "when arguments are empty" do
      it "puts help message to error output" do
        command = Todo::Open.new(arguments: [], output: output, error_output: error_output)
        command.run(repository: repository)
      rescue SystemExit
        # ignore exit
      ensure
        expect(error_output.string).to eq(Todo::Open::HELP_MESSAGE)
      end
    ...
    ["-h", "--help"].each do |flag|
      context "when arguments include '#{flag}' flag" do
        it "puts help message to output" do
          command = Todo::Open.new(arguments: [flag], output: output, error_output: error_output)
          command.run(repository: repository)
          expect(output.string).to eq(Todo::Open::HELP_MESSAGE)
        end
      end
    end
    ...
    context "when arguments include an ID" do
      it "calls FileRepository#open with the ID" do
        ...
        command = Todo::Open.new(arguments: ["1"], output: output, error_output: error_output)
        command.run(repository: repository)
      end
    end
  end
